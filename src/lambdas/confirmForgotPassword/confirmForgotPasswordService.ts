...
export const confirmForgotPasswordService = async (
  req: ConfirmForgotPasswordReqDTO
): Promise<any> => {
  try {
    // FIXME: esto podria ser una libreria asi no repetir codigo
    ...
    const userData = {
      ...
      const response = await client.send(new ConfirmForgotPasswordCommand(userData))
      .then((value) => value)
      .catch((err) => {
        logger.error({
          msg:
            "Error in forgotPasswordService - sendCode with message: " +
            err.msg,
          error: err,
        });
        throw err;
  ...
  try {
    ...
    const data = await kms
      ...
      .catch((err) => {
        logger.error({
          msg:
            "Error in forgotPasswordService - getClientId with message: " +
            err.msg,
          error: err,
        });
        throw err;
    ...
    logger.error({
      msg:
        "tryCatch Error in forgotPasswordService - getClientId with message: " + error.msg,
      error: error,
    });
    throw error;
  }
